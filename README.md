# Fundamentals of Accelerated Computing with CUDA Python

Welcome to the **Fundamentals of Accelerated Computing with CUDA Python** repository! This repository contains all the materials, code samples, and exercises for learning the basics of accelerated computing using CUDA and Python.

## Table of Contents

- [Introduction](#introduction)
- [Prerequisites](#prerequisites)
- [About](#about)


## Introduction

This course is designed to introduce you to the world of accelerated computing using CUDA and Python. You will learn how to leverage the power of GPUs to speed up your computations and solve complex problems more efficiently.

## Getting Started

To get started with this repository, clone it to your local machine using the following command:

```sh
git clone https://github.com/batulaiko/Fundamentals-of-Accelerated-Computing-with-CUDA-Python.git
```

Libraries:
`pip install numba numpy`

## About

The course aims to show what just-in-time compiler is and how we can use it on GPU Acceleration and Efficient Memory Usage on GPU.


![CUDA Python](https://developer.nvidia.com/sites/default/files/akamai/cuda/images/CUDA-Logo.png)

